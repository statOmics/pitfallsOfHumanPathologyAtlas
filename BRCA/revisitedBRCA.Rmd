...
BRCA_counts <- BRCA_counts[,!BRCA_counts$is_ffpe]
BRCA_counts <- BRCA_counts[,which(BRCA_counts$gender == "female")]

BRCA_counts <- BRCA_counts[,BRCA_counts$shortLetterCode=="TP"]

BRCA_counts$days_to_death[grep("Alive",BRCA_counts$vital_status,ignore.case = TRUE)]<- -Inf
BRCA_counts$days_to_last_follow_up[grep("Dead",BRCA_counts$vital_status,ignore.case = TRUE)]<- -Inf

BRCA_counts <-BRCA_counts[, !is.na(BRCA_counts$days_to_death) & !is.na(BRCA_counts$days_to_last_follow_up) & !is.na(BRCA_counts$age_at_diagnosis)]
```

## Generate survival object

```{r}
ttimeBrca <- BRCA_counts$days_to_death

status <- ttimeBrca >= 0
ttimeBrca[!status] <- BRCA_counts$days_to_last_follow_up[!status]

ttimeBrca <- ttimeBrca/365
ttimeBrca <- Surv(ttimeBrca, status)

BRCA_counts$age_at_diagnosis <- BRCA_counts$age_at_diagnosis/365
```

## CPH model for age effect only

```{r}
## Cox model for the effect of age on survival
baseLine_age <- coxph(ttimeBrca ~ qspline(BRCA_counts$age_at_diagnosis,df=4))

b <- anova(baseLine_age)[2,4]
b # extremely significant association between age and survival

# plotting the baseline age model
ordAge <- order(BRCA_counts$age_at_diagnosis)

#png("figuresBRCA/age_confounder.png")
plot(BRCA_counts$age_at_diagnosis[ordAge],predict(baseLine_age,type="terms")[ordAge,1], xlab="Age", ylab="contribution", main=paste0("Age only (p =",format(anova(baseLine_age)[2,4],digits=2),")"),type="l",ylim = c(-0.5,2.5))
rug(BRCA_counts$age_at_diagnosis)
#dev.off()
```
...
```{r}
counts <- DGEList(assays(BRCA_counts) [["HTSeq - Counts"]], group=BRCA_counts$shortLetterCode) ## all belong to same group

keep <- filterByExpr(counts)
summary(keep)
## 38664 filtered out, 18299 retained
...
counts <- counts[which(rownames(counts)%in%BRCA_Uhlen$EnsemblIDs),]
```

### Visualize the CoxPH model for the IL27RA gene,both withoutand with correction for the confounder age

```{r}
gene <- "ENSG00000104998" # IL27RA
#png("figuresBRCA/gene_IL27RA_CoxPH.png") # for printing to folder
i <- which(row.names(counts) == "ENSG00000104998")

genename <- rowData(BRCA_counts)[which(rowData(BRCA_counts)$ensembl_gene_id == gene),2]

par(mfrow=c(1,2))

## Cox model for the effect of gene expression on survival
geneModelOnly <- coxph(ttimeBrca ~ qspline(counts[i,],df=4))
geneModelWithBase <- coxph(ttimeBrca ~ qspline(counts[i,],df=4)+qspline(BRCA_counts$age_at_diagnosis, df=4))

ord <- order(counts[i, ])
ordAge <- order(BRCA_counts$age_at_diagnosis)
  
ylimGene = range(c(predict(geneModelOnly,type="terms")[ord,1], predict(geneModelWithBase,type="terms")[ord, 1]))
  
ylimAge = range(c(predict(baseLine_age,type="terms")[ordAge,1],predict(geneModelWithBase,type="terms")[ordAge,2]))

## plotting the genemodel
plot(counts[i,ord], predict(geneModelOnly,type="terms")[ord,1], xlab="log expression", ylab="contribution", main = paste0(genename," only (p =", format(anova(geneModelOnly)[2,4],digits=2),")"), type = "l", ylim = ylimGene,cex.main=0.7)

rug(counts[i,])
  
# plotting the gene model corrected for age
plot(counts[i,ord],predict(geneModelWithBase,type="terms")[ord,1], xlab="log expression", ylab="contribution", main=paste0(genename," age corrected (p =", format(anova(geneModelWithBase,baseLine_age)[2,4], digits=2),")"), type="l", ylim=ylimGene,cex.main=0.7)

rug(counts[i,])
#dev.off() # for printing to folder
```

# Main analysis
## Analysis with spline predictor for gene expression while correcting for age (qspline)

Here, we assess the association between gene expression and survival, after correcting for the confounder age, for each gene. The progress of the analysis can be followed below.

```{r}
pvalList_BRCA <- data.frame(matrix(NA, nrow = nrow(counts), ncol = 4))
rownames(pvalList_BRCA) <- rownames(counts)

baseLine_age <- coxph(ttimeBrca ~ qspline(BRCA_counts$age_at_diagnosis,df=4))
b <- anova(baseLine_age)[2,4]
pvalList_BRCA[,2] <- b ## pval for age only
...
get_pvals <- function(i){
    ...
    c <- anova(geneModelWithBase,baseLine_age)[2,3] # test statistic after age correction
    d <- anova(geneModelWithBase,baseLine_age)[2,4] # p-value after age ccorrection

    return(c(a,c,d))
}

pvalList_BRCA[,c(1,3,4)]  <- t(pbsapply(1:nrow(counts), get_pvals))
```

### Correct for multiple testing

```{r}
#Correct for multiple testings
pvalList_BRCA$gene_corrected_FDR <- p.adjust(p = pvalList_BRCA[, 4], method = "BH")
colnames(pvalList_BRCA) <- c("gene_only", "age_only","test_stat", "gene_corrected","gene_corrected_FDR")

length(which(pvalList_BRCA$gene_corrected <= 0.05))
length(which(pvalList_BRCA$gene_corrected_FDR <= 0.05)) ## no significant genes after age and multiple testing correction
```

Histogram of p-values (not FDR-corrected) for each gene after correction for the confounder age.

```{r}
png(filename = "figuresBRCA/pvalue_distribution_Cox.png")
hist(pvalList_BRCA$gene_corrected,xlim = c(0,1), main="Distribution of CoxPH age-corrected p-values",cex.main=0.8, breaks = 40)
dev.off()
```
...
```{r}
png(filename = "figuresBRCA/chisq_distribution_Cox.png")
chisq <- qchisq(1-pvalList_BRCA$gene_corrected, 4) ## convert p-values to chisq values
...
Efron describes four reasons why the theoretical null distribution may fail; (I) failed mathematical assumptions, (II) correlation across genes, (III) correlation across patients, and (IV) unobserved confounders in observational studies. The data structure in our study is depicted in Supplementary Figure S7 and illustrates reasons (II), (III) and (IV).
...
```{r}
#png(filename = paste0("figuresBRCA/lfdr_qspline_qspline.png"))
current_plot <- locfdr(qnorm(1-pvalList_BRCA$gene_corrected), main="qspline count - qspline age correction - LRT")
#dev.off()
```

# Why does the theoretical null of the CPH-LR fail?

## Analysis with spline predictor for gene expression (no age correction)
...
```{r}
#png(filename = paste0("figuresBRCA/lfdr_qspline_no.png"))
current_plot <- locfdr(qnorm(1-pval_2), main="qspline count - no correction")
#dev.off()
```

## Analysis with linear and quadratic predictor for gene expression (no age correction)
...
baseline <- coxph(ttimeBrca ~ 1)

get_pvals <- function(i){

    geneModel <- coxph(ttimeBrca ~ counts[i,] + I(counts[i,]^2))
...
```{r}
#png(filename = paste0("figuresBRCA/lfdr_linquad_no.png"))
...
get_pvals <- function(i){
    ...
    geneModelOnly <- coxph(ttimeBrca ~ qspline(counts_resample[i,],df=4))
    return(anova(geneModelOnly)[2,4])
}
...
```{r}
#png(filename = paste0("figuresBRCA/lfdr_qspline_no_breakGene.png"))
...
get_pvals <- function(i){
    ...
    geneModel <- coxph(ttimeBrca ~ counts_resample[i,] + I(counts_resample[i,]^2))
    return(anova(geneModel, baseline)[2,4])
}
...
```{r}
png(filename = paste0("figuresBRCA/lfdr_linquad_no_breakGene.png"))
...
get_pvals <- function(i){
    ...
    geneModelOnly <- coxph(ttimeBrca_current ~ counts[i,] + I(counts[i,]^2))
...
for (i in 1:6) {
  ...
  ttimeBrca_current <- sample(ttimeBrca)
  print(head(ttimeBrca_current)) # print to show if the permuta

  baseline_current <- coxph(ttimeBrca_current ~ 1)

  pval_6 <- t(pbsapply(1:nrow(counts), get_pvals))
#  png(filename = paste0("figuresBRCA/lfdr_linquad_no_breakSurvival", i, ".png"))
  current_plot <- locfdr(qnorm(1-pval_6), main="linear-quadratic count - no correction - break survival")
#  dev.off()
}
```

Note, that the empirical null for the six permutations does not indicate substantial deviations from the standard normal distribution. Indicating that the correlation between genes does not induce a failure of the theoretical null in the BRCA example.

Hence, for the BRCA data we can conclude that the failure of the null is mainly induced by unmeasured confounders.

Finally, we will evaluate if we can address this failure by correcting for the only confounder age, which is available in the public meta data.   

# Analysis with linear-quadratic count while correcting for age (qspline) using LRT
...
get_pvals <- function(i){
    ...
    geneModelWithBase <- coxph(ttimeBrca ~ counts[i,] + I(counts[i,]^2) + qspline(BRCA_counts$age_at_diagnosis,df=4))
    pval_current <- anova(geneModelWithBase,baseLine_age)[2,4] ## take pvals
...
```{r}
#png(filename = paste0("figuresBRCA/lfdr_linquad_qspline.png"))
...
```{r}
pvalList_BRCA$gene_corrected_lfdr <- current_plot$fdr
...
```{r}
pvalList_BRCA$KaplanMeier <- BRCA_Uhlen[match(rownames(pvalList_BRCA),BRCA_Uhlen$EnsemblIDs),"log-rank.P.Values"]

'%!in%' <- function(x,y)!('%in%'(x,y))

hlp_matrix <- matrix(data = NA, nrow = length(which(BRCA_Uhlen$EnsemblIDs %!in% rownames(pvalList_BRCA))), ncol = 7)

colnames(hlp_matrix) <- c("gene_only", "age_only", "test_stat", "gene_corrected","gene_corrected_FDR","gene_corrected_lfdr", "KaplanMeier")

rownames(hlp_matrix) <- BRCA_Uhlen$EnsemblIDs[which(BRCA_Uhlen$EnsemblIDs %!in% rownames(pvalList_BRCA))]

pvalList_BRCA <- rbind(pvalList_BRCA,hlp_matrix)

pvalList_BRCA$KaplanMeier <- BRCA_Uhlen[match(rownames(pvalList_BRCA),BRCA_Uhlen$EnsemblIDs),"log-rank.P.Values"]

pvalList_BRCA$Prognostic.Types <- BRCA_Uhlen[match(rownames(pvalList_BRCA),BRCA_Uhlen$EnsemblIDs),"Prognostic.Types"]

pvalList_BRCA$rank_our <- rank(pvalList_BRCA$gene_corrected,na.last = 'keep') ## rank of the gene in our analysis
pvalList_BRCA$rank_Uhlen <- rank(pvalList_BRCA$KaplanMeier,na.last = 'keep') ## rank of the gene in the original analysis
```
...
```{r}
pvalList_BRCA$Symbols <- BRCA_Uhlen[match(rownames(pvalList_BRCA),BRCA_Uhlen$EnsemblIDs),"Symbols"]

pvalList_BRCA <- pvalList_BRCA[,c(11,1:10)] # format

top10_Uhlen <- pvalList_BRCA[order(pvalList_BRCA$rank_Uhlen)[1:10],]
top10_our <- pvalList_BRCA[order(pvalList_BRCA$rank_our)[1:10],]

write.xlsx(pvalList_BRCA,"dataBRCA/pvalList_BRCA.xlsx", rowNames=TRUE)
write.xlsx(top10_Uhlen,"dataBRCA/top10_Uhlen_BRCA.xlsx", rowNames=TRUE)
write.xlsx(top10_our,"dataBRCA/top10_our_BRCA.xlsx", rowNames=TRUE)
```
